class Api::V1::CalendarsRebookController < Api::V1::ApplicationController
  ...
  def get_companies_data(zipcode,properties,p_total_hours)
    ...
    @companies = Company.find_by(:id => params[:company_id])
    ...
    @days = Company.where(id: @companies.values.flatten.pluck(:company_id)).map{|company| company.time_slots.map{|ts| ts if (ts.ending_time.hour - ts.starting_time.hour) >= p_total_hours.to_i}.reject{|ele| ele.nil?}.group_by(&:user_slotable_id)}.reject{|day| day.blank?}
    ...
    return @companies_with_green, @companies_with_orange
  end

class Staff::JobsController < Staff::BaseStaffController
  ...
  def list
    @tasks = Job.all
    @tasksassignedstaff = Job.assigned_jobs(@company.id).where("staff_id = ?", current_staff.id).reverse_order
    @taskscompletedstaff = Job.completed_jobs(@company.id).reverse_order
  end
  ...
  def add_staff
      ...
      @users = @users.map{|s| s if (s.time_slots.where(working_date: @tasks.booking_date).present? && Job.where(booking_date: @tasks.booking_date).where.not(staff_id: s.id).or(Job.where(booking_date: @tasks.booking_date).where(staff_id: nil)).present?)}.compact
      respond_to do |format|
        format.html
        format.js
      end
  ...
  def jobs_update
      ...
      if @jobs.update(job_params)
        format.html { redirect_to staff_jobs_add_staff_path, notice: 'Job was successfully updated.' }
        format.js
  ...
  def create
    ...
    respond_to do |format|
      if @job.save
        format.html { redirect_to staff_jobs_list_manual_path, notice: 'Job was successfully created.' }
        format.json { render :show, status: :created, location: @job }
        format.js { redirect_to staff_jobs_list_manual_path, notice: 'Job was successfully created.' }
      else
  ...
  private
    ...
    def job_params
      params.require(:job).permit(:user_id, :task_id, :company_id, :booking_date, :booking_time, :price, :manual_customers_id, :is_manual_booking, :status, :view_job_id, :post_date, :category_id, :address1, :city, :country, :latitude, :longitude, :address, :properties, :zipcode, :staff_id)
    end
